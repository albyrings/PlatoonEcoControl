...
for t=1:length(Tempo)-1
    u(t)=-gain_k*sign(sigma(t))+(1/h_1)*(h_2*(x(2,t))^2+h_3*x(2,t)+h_0); % u sliding mode control u=(ueq-ksign(s))
...
% event trigger
    if abs(vel_star(t+1)-y(t+1))>=thr1 && tt_ottimizzazione-tt_re_ottimizzazione>=thr2
        d_tr(1)=x(1,t+1); 
        if d_tr(1)>d(1) && d_tr(1)<=d(2) % tra 0 e 300
        ...
        elseif d_tr(1)>d(2) && d_tr(1)<=d(3) %tra 300 e 600
        ...
        elseif d_tr(1)>d(3) && d_tr(1)<=d(4) %tra 600 e 900
        ...
        elseif d_tr(1)>d(4) && d_tr(1)<=d(5) %tra 900 e 1200
        ...
        elseif d_tr(1)>d(5) && d_tr(1)<=d(6) %tra 1200 e 1550
...
for t=1:length(Tempo)
    plot_ref(t)=x(2,t)-sigma(t);
end

% % x_1_plot=x(1,:)/(1/passo);
figure(1)
...
plot(Tempo,plot_ref-y(1,:),'Linewidth',1.5);
hold on
...
%% SAVE PICTURES
ScriptForFigToPdfFun(fig1,'fig2',0)
ScriptForFigToPdfFun(fig3,'fig3',0)
ScriptForFigToPdfFun(fig5,'fig4',0)
ScriptForFigToPdfFun(fig6,'fig5',0)