clear; clc; close all;
%%%% come prima ma fa tutto in un file
%% Esegui la simulazione iniziale con il leader di default (veicolo #1)
...
function run_optimizer_and_plot(leader_vehicle)
    % Questo script locale ricostruisce l'intero flusso:
    % 1) Calcolo del percorso ottimo via Dijkstra
    % 2) Simulazione ODE
    % 3) Plots e check passaggi col rosso (che se trovati, rilanciano l'optimizer)

    disp(['[INFO] Eseguo run_optimizer_and_plot con Leader = Veicolo ',num2str(leader_vehicle)]);
    ...
    %% Disegno i limiti min/max su t_min e t_max (vel pruning)
    ...
    fprintf('>>> Leader = %d, Costo energetico ottimo: %f\n', leader_vehicle, cost);
    ...
    %% SECONDA PARTE: SIMULAZIONE ODE (figura 2 e 3)
    b1_for_delta = 450;
    b2_for_delta = 450;
    b3_for_delta = 1; 
    b4_for_delta = 3;
    delta_disturbo = @(t) 0*(b1_for_delta + b2_for_delta*( ...
    ...
    [t_sim, x_sim] = ode45(@(t,x) system_dynamics_new_platoon( ...
        t, x, n_vehicles, m_vehicles, delta_disturbo, ...
    ...
    figure('Name',['Nuovo plot: Semafori+Veicoli (Leader=' num2str(leader_vehicle) ')']); hold on;
    ...
    title(['Mappa semafori e posizioni veicoli (Leader=' num2str(leader_vehicle) ')']);
...
function dx = system_dynamics_new_platoon(t, x, n_vehicles, m, delta_func, ...
    ...
    K_p_speed, K_i_speed, K_d_speed, ...
    K_p_dist, K_i_dist, K_d_dist, leader_vehicle)
    % Come "system_dynamics", ma con la possibilità di scegliere il leader
    % ( leader_vehicle ) su cui usare la logica di "leader".
    ...
    for i = 1:n_vehicles
        ...
        if i == leader_vehicle
            ...
            % Follower
            % Segue il veicolo i-1, se i>1, altrimenti se i=1 ma non è leader_vehicle,
            % si può definire un convoglio a parte (qui semplifichiamo, supponendo i>1).
            if i>1
                distance = x(i) - x(i-1);
            else
...
function rerun_optimizer_for_new_leader(violating_vehicle)
    % Rilanci la procedura come se "violating_vehicle" fosse leader
