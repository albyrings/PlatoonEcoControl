...
function diff_plot_per_vehicle()
    ...
    % tra la traiettoria ottimizzata e quella simulata, interpolata su t_sim.
    ...
    for v = 1:n_vehicles
        ...
        if v == leader
            ...
            for idx = 2:length(opt_t)
                % Calcolo del gap richiesto (semplice, come negli altri grafici)
                dt = opt_t(idx) - opt_t(idx-1);
                v_avg = (opt_d_leader(idx) - opt_d_leader(idx-1)) / dt; %#ok<NASGU>
