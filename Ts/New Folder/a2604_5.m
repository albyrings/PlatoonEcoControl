...
function run_optimizer_and_plot(leader_vehicle, time_offset, start_position)
    ...
    % Se start_position non è specificato, inizia dalla posizione 0
    if nargin < 3
        start_position = 0;
    end
    ...
    % Modifica il calcolo considerando la posizione di partenza
    remaining_distance = final_distance - start_position;
    
    % Esempio di pruning
    [t_min, t_max] = velocity_pruning(traffic_lights, tf, final_distance, v_min, v_max, start_position);
    d = [traffic_lights.distance]; 
    
    % Filtra i semafori che sono dopo la posizione di partenza
    valid_intersections = find(d > start_position);
    d = d(valid_intersections);
    filtered_lights = traffic_lights(valid_intersections);
    nIntersections = length(filtered_lights);

    % Costruzione nodi
    Nodes = struct('id', {}, 't', {}, 'd', {}, 'int', {});
    nodeId = 1;
    Nodes(nodeId) = struct('id', nodeId, 't', 0, 'd', start_position, 'int', 0); 
    nodeId=nodeId+1;

    for i=1:nIntersections
        light = filtered_lights(i);
            ...
            if lvlB==lvlA+1
                if Nodes(j).t>Nodes(i).t && Nodes(j).d>Nodes(i).d
                    if Nodes(j).int>0 && Nodes(j).int<=nIntersections
                        if ~is_green(filtered_lights(Nodes(j).int), Nodes(j).t)
                            continue;
                        end
                    end
    ...
    % Inizializziamo le condizioni iniziali in base alla posizione di partenza
    x0=zeros(2*n_vehicles,1);
    for i=1:n_vehicles
        if i==leader_vehicle
            x0(i) = start_position;
        elseif i < leader_vehicle
    ...
    % Se siamo in un nuovo plotone, inizializza le velocità con una velocità ragionevole
    if start_position > 0
        mean_target_speed = mean(v_targets);
        for i=1:n_vehicles
            x0(n_vehicles + i) = mean_target_speed * 0.8; % 80% della velocità media target
        end
    end
...
function check_red_light_violations(t_abs, x_sim, traffic_lights, T)
    ...
    for L = 1:length(traffic_lights)
        ...
        % Array per i veicoli che hanno già attraversato e quelli che devono fermarsi
        vehicles_passed = [];
        vehicles_to_stop = [];
        passing_with_red = false;
        
        % Per ogni veicolo, controlla posizione attuale e attraversamenti
        for v = 1:n_vehicles
            ...
            % Controlla se il veicolo ha attraversato il semaforo
            if last_pos >= light_d
                % È passato attraverso il semaforo
                vehicles_passed = [vehicles_passed, v];
                
                % Verifica se è passato col rosso
                ...
                if ~isempty(cross_idx)
                    ...
                    if ~is_green(traffic_lights(L), cross_time)
                        % È passato col rosso
                        passing_with_red = true;
                        
                        % Notifica (se non già rilevata)
                ...
                % Non ha ancora attraversato il semaforo
                vehicles_to_stop = [vehicles_to_stop, v];
            end
        end
        
        % Se almeno un veicolo è passato col rosso e ci sono veicoli che devono fermarsi
        if passing_with_red && ~isempty(vehicles_to_stop)
            % Scegli il primo veicolo come nuovo leader
            vehicles_to_stop = sort(vehicles_to_stop);
            new_leader = vehicles_to_stop(1);
            
            fprintf('\n>> Plotone diviso al semaforo %d: veicoli %s sono passati (con rosso), veicoli %s si fermano\n', ...
                    L, mat2str(vehicles_passed), mat2str(vehicles_to_stop));
            fprintf('>> Veicolo %d diventa leader del nuovo plotone!\n', new_leader);
            
            % Aggiorna i veicoli che sono stati divisi
            SIM_RUNS{current_run}.splittedVehicles = vehicles_to_stop;
            
            % Ottieni l'ultimo tempo di attraversamento con il rosso
            cross_time = t_abs(end);
            for v = vehicles_passed
                cross_idx = find(x_sim(:, v) >= light_d, 1);
                if ~isempty(cross_idx)
                    cross_time = t_abs(cross_idx);
                    break;
                end
            ...
            % Riavvia la simulazione con il nuovo leader, attesa al semaforo
            rerun_optimizer_for_red_light(new_leader, light_d, L, cross_time);
            return;
        end
        
        % Anche se nessuno è passato col rosso, controlla se il leader sta per passarlo
        % con previsione anticipata (implementazione predittiva)
        if ~isempty(vehicles_to_stop) && ~isempty(vehicles_passed)
            leader = SIM_RUNS{current_run}.leader;
            if ismember(leader, vehicles_passed)
                % Il leader è già passato, controlla se sta per esserci un rosso
                for v = vehicles_to_stop
                    pos_v = x_sim(:, v);
                    vel_v = x_sim(:, n_vehicles + v);
                    
                    % Tempo stimato per arrivare al semaforo
                    dist_to_light = light_d - pos_v(end);
                    if vel_v(end) > 0.1  % Se la velocità è significativa
                        time_to_light = dist_to_light / vel_v(end);
                        arrival_time = t_abs(end) + time_to_light;
                        
                        if ~is_green(traffic_lights(L), arrival_time) && time_to_light < 5  % Se arriverà col rosso nei prossimi 5 secondi
                            % Dividi il plotone in modo preventivo
                            fprintf('\n[ANTICIPO] Semaforo %d diventerà rosso quando il veicolo %d arriverà (t=%.2f s)\n', ...
                                L, v, arrival_time);
                            
                            vehicles_to_stop = sort(vehicles_to_stop);
                            new_leader = vehicles_to_stop(1);
                            
                            fprintf('>> Plotone diviso preventivamente: veicoli %s continuano, veicoli %s si fermeranno\n', ...
                                    mat2str(vehicles_passed), mat2str(vehicles_to_stop));
                            
                            SIM_RUNS{current_run}.splittedVehicles = vehicles_to_stop;
                            rerun_optimizer_for_red_light(new_leader, light_d, L, t_abs(end));
                            return;
                        end
                    end
                end
...
function result = is_in_current_platoon(vehicle_id, current_run)
    ...
    % Per il primo run
    if current_run == 1
        % Nel primo run, tutti i veicoli sono nel plotone originale
        % a meno che non siano stati splittati
    ...
    % Per i run successivi
    result = false;  % Default: non è nel plotone corrente
    
    % Un veicolo è nel plotone corrente se è stato splittato dal plotone precedente
    prev_run = current_run - 1;
    if isfield(SIM_RUNS{prev_run}, 'splittedVehicles') && ...
       ~isempty(SIM_RUNS{prev_run}.splittedVehicles) && ...
       ismember(vehicle_id, SIM_RUNS{prev_run}.splittedVehicles)
        
        % Ma non deve essere stato ulteriormente splittato nel run corrente
        if isfield(SIM_RUNS{current_run}, 'splittedVehicles') && ...
           ...
           ismember(vehicle_id, SIM_RUNS{current_run}.splittedVehicles)
            result = false;  % È stato ulteriormente splittato
        else
            result = true;   % È nel plotone corrente
        end
    end
end

function rerun_optimizer_for_red_light(new_leader, semaphore_position, semaphore_id, cross_time)
    clear system_dynamics_new_platoon
    clear get_current_v_target_indexed
    ...
    % Trova il prossimo tempo di verde per questo semaforo
    traffic_lights = SIM_RUNS{1}.traffic_lights;
    light = traffic_lights(semaphore_id);
    next_green_time = next_green(light, cross_time);
    
    % Mostra informazioni sulla creazione del nuovo plotone
    current_platoon = N_PLATOON + 1;
    disp(['[INFO] SEMAFORO ROSSO: Creazione NUOVO PLOTONE ' num2str(current_platoon) ' con LEADER=' ...
          num2str(new_leader) ', in attesa al semaforo ' num2str(semaphore_id) ...
          ' (posizione=' num2str(semaphore_position) ...
          'm, riparte al verde t=' num2str(next_green_time) 's)']);

    % Incrementa il contatore di plotoni
    N_PLATOON = N_PLATOON + 1;
    
    % Esegui la simulazione con il nuovo leader, partendo dalla posizione del semaforo
    % e dal tempo in cui diventa verde
    run_optimizer_and_plot(new_leader, next_green_time, semaphore_position);
    
    % Pulisci lo stato delle violazioni e dei trigger
    clear check_red_light_violations
...
function [t_min, t_max] = velocity_pruning(traffic_lights, tf, final_distance, v_min, v_max, start_position)
    if nargin < 6
        start_position = 0;
    end
    
    % Filtra i semafori che sono dopo la posizione di partenza
    valid_semaphores = find([traffic_lights.distance] > start_position);
    filtered_lights = traffic_lights(valid_semaphores);
    n = length(filtered_lights);
    
    d = [filtered_lights.distance];
    t_min = zeros(1,n); 
    t_max = zeros(1,n);
    
    % Calcola t_min e t_max considerando la posizione di partenza
    for i = 1:n
        if i == 1
            t_min(1) = (d(1) - start_position) / v_max; 
            t_max(1) = (d(1) - start_position) / v_min;
        else
            dist_inc = d(i) - d(i-1);
        ...
        t_min(i) = next_green(filtered_lights(i), t_min(i));
        t_max(i) = prev_green(filtered_lights(i), t_max(i));
    end
    
    % Back-propagation dei vincoli
    for i = n:-1:2
        needed_t = (d(i) - d(i-1)) / v_max;
        if t_max(i) > t_max(i-1) + needed_t
            t_max(i-1) = t_max(i) - needed_t;
            t_max(i-1) = prev_green(filtered_lights(i-1), t_max(i-1));
        end
    end
end
...
function check_velocity_triggers(t_abs, x_sim, traffic_lights)
    ...
    % VERIFICA SEMAFORI ROSSI ANTICIPATA
    % Identifica i veicoli che stanno per passare con il rosso
    for L = 1:length(traffic_lights)
        light_d = traffic_lights(L).distance;
        vehicles_passed = [];
        vehicles_to_stop = [];
        ...
        for v = 1:n_vehicles
            % Salta i veicoli che non sono in questo plotone o sono già leader
            if ~is_in_current_platoon(v, current_run) || v == runData.leader
                continue;
            end
            ...
            if pos_v(current_idx) >= light_d
                vehicles_passed = [vehicles_passed, v];
            else
                vehicles_to_stop = [vehicles_to_stop, v];
                
                % Controlla se sta per passare col rosso
                if pos_v(current_idx) < light_d && v_vel(current_idx) > 0.1
                    time_to_light = (light_d - pos_v(current_idx)) / v_vel(current_idx);
                    
                    if time_to_light < 5  % Previsione per i prossimi 5 secondi
                        cross_time = t_abs(current_idx) + time_to_light;
                        
                        if ~is_green(traffic_lights(L), cross_time) && ~trigger_detected
                            fprintf('\n[TRIGGER] Veicolo %d sta per passare col rosso a incrocio %d (t=%.2f s)\n', ...
                                    v, L, cross_time);
                            
                            trigger_detected = true;
                            
                            % Determina il nuovo leader
                            vehicles_to_stop = sort(vehicles_to_stop);
                            new_leader = vehicles_to_stop(1);
                            
                            fprintf('>> Plotone diviso anticipatamente: veicolo %d diventa leader\n', new_leader);
                            
                            SIM_RUNS{current_run}.splittedVehicles = vehicles_to_stop;
                            rerun_optimizer_for_red_light(new_leader, light_d, L, t_abs(current_idx));
    ...
    % VERIFICA TRIGGER DI VELOCITÀ
    for v = 1:n_vehicles
        % Salta i veicoli che non fanno parte di questo plotone o sono già leader
        if ~is_in_current_platoon(v, current_run) || v == runData.leader
            continue;
        end
        ...
        if any(trigger_state) && ~trigger_detected
            ...
            % Trova tutti i veicoli che seguono questo veicolo
            following_vehicles = v:n_vehicles;
            SIM_RUNS{current_run}.splittedVehicles = following_vehicles;
            
            % Avvia la simulazione con il nuovo plotone
...
function rerun_optimizer_for_trigger(trigger_vehicle, trigger_time)
    ...
    run_optimizer_and_plot(trigger_vehicle, start_offset);
...
function rerun_optimizer_for_new_leader(violating_vehicle, T)
    % Aggiornala per chiamare la nuova funzione
    global SIM_RUNS
    current_run = length(SIM_RUNS);
    
    % Trova il semaforo che è stato violato
    runData = SIM_RUNS{current_run};
    for v = 1:size(runData.x,2)/2
        for L = 1:length(runData.traffic_lights)
            if v == violating_vehicle
                light_d = runData.traffic_lights(L).distance;
                cross_idx = find(runData.x(:, v) >= light_d, 1);
                if ~isempty(cross_idx)
                    cross_time = runData.t(cross_idx);
                    if ~is_green(runData.traffic_lights(L), cross_time)
                        % Trovato! Usa la nuova funzione
                        rerun_optimizer_for_red_light(violating_vehicle, light_d, L, cross_time);
                        return;
                    end
                end
            end
    ...
    % Se non troviamo informazioni specifiche, usa il vecchio approccio
    clear system_dynamics_new_platoon
    clear get_current_v_target_indexed
    
    global N_PLATOON
    
    disp(['[INFO] Creazione NUOVO PLOTONE ' num2str(N_PLATOON+1) ' con LEADER=' ...
