...
SIM_RUNS = {};
N_PLATOON = 1;
set(groot,'DefaultTextInterpreter','latex','DefaultLegendInterpreter','latex','defaultAxesTickLabelInterpreter','latex'),
...
%    Se nel run corrente è destinato a staccarsi, la sua linea si ferma
%    esattamente all’istante di split (offset del run successivo),
%    esattamente come nel plot “optimal”.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plot_real_trajectories()
    ...
    for r = 1:length(SIM_RUNS)
        ...
        % veicoli che dal run r si separeranno in r+1
        splitV = [];  splitT = Inf;
        ...
        % veicoli da plottare in questo run
        pv_all = union(platoonV, splitV);
        pv_all = pv_all(:).';                       % vettore riga

        %–– loop veicoli ––––––––––––––––––––––––––––––––––––––––––––––––
        for v = pv_all
            col = veh_colors(mod(v-1,size(veh_colors,1))+1,:);

            % maschera temporale: tronca PRIMA dello split
            if ismember(v, splitV)
                mask = t_sim < splitT;              % nota il “<” stretto
            else
                mask = true(size(t_sim));
            end
            if ~any(mask), continue; end            % sicurezza
            ...
            legH(end+1) = h;
            if ismember(v, splitV)
                legT{end+1} = sprintf('V%d (Real pre-split, run %d)', v, r);
            else
                legT{end+1} = sprintf('V%d (Real, run %d)', v, r);
            end

            % opzionale: pezzetto finale interpolato esattamente a splitT
            if ismember(v, splitV)
                lastIdx = find(mask,1,'last');
                if ~isempty(lastIdx) && t_sim(lastIdx) < splitT
                    pos_split = interp1(t_sim, x_sim(:,v), splitT, 'linear');
                    plot([t_sim(lastIdx) splitT], ...
                         [x_sim(lastIdx,v) pos_split], ...
                         '-', 'LineWidth', 2, 'Color', col);
                end
            end
        end
    ...
    ScriptForFigToPdfFun(fig1,'real',0);
end
...
function plot_optimal_trajectories_and_lights()
    ...
    fig1 = figure('Name','Optimal Trajectories and Traffic Lights',...
    ...
    xlabel('Time [s]'); ylabel('Position [m]');
    legend(legH, legT,'Location','Best');
    ScriptForFigToPdfFun(fig1,'optimal',0);
end

function save_plot_data(SIM_RUNS)
    if isempty(SIM_RUNS),  warning('SIM_RUNS vuoto: niente da salvare'); return; end
    out.traffic_lights = SIM_RUNS{1}.traffic_lights;   % uguale per tutti i run
    for i = 1:numel(SIM_RUNS)
        run = SIM_RUNS{i};
        out.run(i).t                = run.t;
        out.run(i).x                = run.x;
        out.run(i).opt_t            = run.opt_t;
        out.run(i).opt_d            = run.opt_d;
        out.run(i).leader           = run.leader;
        if isfield(run,'splittedVehicles')
            out.run(i).splittedVehicles = run.splittedVehicles;
        else
            out.run(i).splittedVehicles = [];
        end
    end
    save('plot_data.mat','-struct','out');
    fprintf('[INFO] plot_data.mat salvato con %d run\n', numel(SIM_RUNS));
end