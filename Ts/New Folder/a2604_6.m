...
% (Modifica della funzione run_optimizer_and_plot per il percorso sicuro)
function run_optimizer_and_plot(leader_vehicle, time_offset)
    ...
    traffic_lights = [
        ...
        create_traffic_light(1200,  0, 15, T)
...
function run_optimizer_from_traffic_light(leader_vehicle, start_time, start_position, traffic_light)
    ...
    % Se non ci sono più semafori davanti, utilizza solo la destinazione finale
    if isempty(remaining_lights)
        % Caso semplice: solo destinazione finale
        ...
        % Costruisci solo nodi iniziale e finale
        Nodes = struct('id', {}, 't', {}, 'd', {}, 'int', {});
        Nodes(1) = struct('id', 1, 't', start_time, 'd', start_position, 'int', 0);
        Nodes(2) = struct('id', 2, 't', tf + start_time, 'd', final_distance, 'int', 1);
        nNodes = 2;
        
        % Costruisci un solo arco
        Edges = struct('from', {}, 'to', {}, 'w', {});
        ...
        % Caso normale con semafori rimanenti
        d = [remaining_lights.distance];
        nIntersections = length(remaining_lights);
        
        % Calcola i limiti di tempo per ogni semaforo
        ...
        for i = 1:nIntersections
            if i == 1
                ...
                dist_from_start = d(i) - d(i-1);
                dist_from_start = d(i) - start_position; % Da calcolare correttamente in base al precedente
            end
        ...
        for i = 1:nIntersections
            ...
            for k = 0:ceil(tf / light.cycle_time)
                ...
                if light.green_start <= light.green_end
                    ...
                    if abs_green_start <= start_time + tf
                        overlap_start = max(abs_green_start, t_min(i));
                        overlap_end = min(abs_green_end, t_max(i));
                    ...
                    % Gestione del caso in cui il verde attraversa la mezzanotte del ciclo
                    abs_green_start_1 = cycle_start + light.green_start;
                    abs_green_end_1 = cycle_start + light.cycle_time;
                    
                    if abs_green_start_1 <= start_time + tf
                        ov_start = max(abs_green_start_1, t_min(i));
                        ov_end = min(abs_green_end_1, t_max(i));
                    ...
                    if abs_green_end_2 <= start_time + tf
                        ov_start = max(abs_green_start_2, t_min(i));
                        ov_end = min(abs_green_end_2, t_max(i));
        ...
        for i = 1:nNodes
            ...
            for j = 1:nNodes
                ...
                if lvlB == lvlA + 1
                    if Nodes(j).t > Nodes(i).t && Nodes(j).d > Nodes(i).d
                        if Nodes(j).int > 0 && Nodes(j).int <= nIntersections
                            if ~is_green(remaining_lights(Nodes(j).int), Nodes(j).t)
                                continue;
                            end
                        end
...
% Verifica se è stato trovato un percorso valido% Gestisci correttamente i vari casi di errore
invalid_cost = false;
if isscalar(cost)
    invalid_cost = isinf(cost);
else
    invalid_cost = any(isinf(cost(:)));
end

invalid_path = isempty(path);
if ~invalid_path && isvector(path)
    invalid_path = length(path) < 2;
end

if invalid_cost || invalid_path
    fprintf('[ERROR] Nessun percorso valido trovato. Creo un percorso predefinito sicuro.\n');
    
    % Crea un percorso predefinito da posizione attuale fino a destinazione
    opt_t = [start_time; start_time + 100];
    opt_d = [start_position; final_distance];
    
    % Velocità costante sicura
    speeds = [10]; % 10 m/s è una velocità moderata e sicura
else
    % Percorso valido trovato, procedi normalmente
    opt_nodes = Nodes(path);
    opt_t = arrayfun(@(n)n.t, opt_nodes);
    opt_d = arrayfun(@(n)n.d, opt_nodes);
    ...
    for k = 1:(length(path) - 1)
        ...
        speeds(k) = max(1, d_ / t_); % Assicura una velocità minima positiva
    end
end
...
function [t_min, t_max] = velocity_pruning_from_position(traffic_lights, tf, final_distance, v_min, v_max, start_position, start_time)
    ...
    for i = 1:n
        ...
        t_max(i) = min(t_max(i), tf - (final_distance - d(i)) / v_max);
    if t_max(i) < t_min(i)
        %   Evita intervalli negativi o nulli
        t_max(i) = t_min(i);
    end
    end
