...
function check_red_light_violations(t_abs, x_sim, traffic_lights, T)
    ...
    % MODIFICA: Aumenta la distanza di rilevamento in anticipo
    look_ahead_distance = 300;  % Metri di anticipo per fermarsi prima del semaforo
    stop_margin = 0; 
    ...
    for L = 1:length(traffic_lights)
        ...
        if ~isempty(approaching_idx)
            ...
            % Verifica se il semaforo è o sarà rosso
            if ~is_green(traffic_lights(L), estimated_crossing_time)
                ...
                stop_position = light_d - 0; % n metri prima del semaforo
