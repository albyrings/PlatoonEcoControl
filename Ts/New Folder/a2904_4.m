...
%% =========================================================================
%% Funzione principale per l'ottimizzazione e la simulazione del platoon
%% =========================================================================
function run_optimizer_and_plot(leader_vehicle, time_offset, start_position)
    ...
    % Funzione di perturbazione (può essere modificata o disattivata)
    ...
    % Definizione dei semafori: posizione, inizio verde, fine verde, durata ciclo
    traffic_lights = [
    ...
    % Pruning delle velocità per determinare le finestre temporali ammissibili
    [t_min, t_max] = velocity_pruning(traffic_lights, tf, final_distance, v_min, v_max, start_position, time_offset);
    
    % Costruzione del grafo per l'ottimizzazione
    [Nodes, Edges] = build_graph(traffic_lights, t_min, t_max, tf, final_distance, ...
                                v_min, v_max, b1, b2, start_position, time_offset);
    
    % Algoritmo di Dijkstra per trovare il percorso ottimale
    ...
    % Estrazione dei nodi e dei tempi/distanze ottimali
    ...
    % Calcolo delle velocità target dai segmenti del percorso ottimale
    ...
    for i = 1:n_vehicles
        if i == leader_vehicle
            ...
            x0(i) = start_position - d_init * abs(i - leader_vehicle);
        end
    end
    
    % Simulazione tramite integrazione numerica
...
%% =========================================================================
%% Dinamica del sistema: modello del platoon di veicoli
%% =========================================================================
function dx = system_dynamics_new_platoon(t, x, n_vehicles, m, delta_func, ...
    ...
    for i = 1:n_vehicles
        ...
        if i == leader_vehicle
            % Dinamica del leader: segue la velocità target
            ...
            % Dinamica dei follower: mantengono la distanza di sicurezza
            if i > 1
                dist = x(i-1) - x(i);
            else
                dist = 10;  % Distanza di default se non c'è un veicolo davanti
            end
            v_cur = x(n_vehicles + i);
            d_min_val = 1;
            d_desired = d_min_val + t_CTH * v_cur;  % Distanza desiderata basata su CTH
...
%% =========================================================================
%% Funzioni per gestione semafori e violazioni
%% =========================================================================
function vt = get_current_v_target_indexed(x_leader, traffic_lights, v_targets)
    % Restituisce la velocità target per il leader in base alla posizione
...
function flag = is_green(light, t_in)
    % Ritorna true se al tempo t_in il semaforo light è verde
    ...
    if green_s <= green_e
        ...
        % Verde a cavallo della fine del ciclo
        flag = (phase >= green_s) || (phase <= green_e);
    end
end
...
function check_red_light_violations(t_abs, x_sim, traffic_lights, T)
    ...
    % Distanza di anticipo per il rilevamento
    ...
    for v = 1:n_vehicles
        ...
        for L = 1:length(traffic_lights)
            ...
            % Cerca veicoli che si avvicinano al semaforo
            ...
            if ~isempty(approaching_idx)
                ...
                if ~is_green(traffic_lights(L), estimated_crossing_time)
                    % Evita di rilevare la stessa violazione più volte
                    ...
                    % Usa la funzione di riavvio ottimizzatore dal semaforo
...
function check_velocity_triggers(t_abs, x_sim, traffic_lights)
    ...
    % Tempo di inibizione del trigger all'inizio di un nuovo plotone
    ...
    % Recupera i dati ottimali dal run corrente
    ...
    for v = 1:n_vehicles
        ...
        for L = 1:length(traffic_lights)
            ...
            if pos_v(current_idx) < light_d && pos_v(current_idx) + v_vel(current_idx)*look_ahead_time >= light_d
                ...
                if ~is_green(traffic_lights(L), cross_time) && ~trigger_detected
                    ...
                    SIM_RUNS{current_run}.splittedVehicles = v:n_vehicles;
                    rerun_optimizer_for_trigger(v, t_abs(current_idx), pos_v(current_idx));
        ...
        % Verifica differenze di velocità
...
%% =========================================================================
%% Funzioni di riavvio simulazione dopo split del plotone
%% =========================================================================
function rerun_optimizer_from_traffic_light(vehicle_id, start_time, start_position, ~)
    global SIM_RUNS
    global N_PLATOON
    
    disp(['[INFO] Creazione NUOVO PLOTONE con leader=' num2str(vehicle_id) ...
          ', attesa al semaforo fino a t=' num2str(start_time) ...
          ', posizione=' num2str(start_position)]);
    
    % Salva i dati attuali prima del reset
    old_runs = SIM_RUNS;
    n_old_runs = length(old_runs);
    
    % Reset completo per nuova simulazione
    N_PLATOON = N_PLATOON + 1;
    reset_persistent_variables();
    SIM_RUNS = {};
    
    % Lancia nuova ottimizzazione
    run_optimizer_and_plot(vehicle_id, start_time, start_position);
    
    % Ripristina i runs precedenti
    for i = 1:n_old_runs
        SIM_RUNS{i} = old_runs{i};
    end
end
...
function rerun_optimizer_for_trigger(trigger_vehicle, trigger_time, stop_position)
    ...
    disp(['[INFO] TRIGGER: Nuovo plotone con leader=' num2str(trigger_vehicle) ...
          ', tempo=' num2str(trigger_time) ...
          ', posizione=' num2str(stop_position)]);
    ...
    % Lancia nuova ottimizzazione
    run_optimizer_and_plot(trigger_vehicle, trigger_time, stop_position);
...
%% =========================================================================
%% Funzioni per ottimizzazione percorso e gestione grafo
%% =========================================================================
function [t_min, t_max] = velocity_pruning(traffic_lights, tf, final_distance, v_min, v_max, start_position, time_offset)
    ...
    for i = 1:n
        ...
        t_min(i) = time_offset + dist_from_start/v_max;
        t_max(i) = time_offset + dist_from_start/v_min;

        t_max(i) = min(t_max(i), time_offset + tf - (final_distance - d(i))/v_max);
...
function t_out = prev_green(light, t_in)
    % Allinea t_in al termine del periodo di verde precedente
    cycle = light.cycle_time;
    offset = light.offset;
    green_s = light.green_start;
    green_e = light.green_end;
...
function [Nodes, Edges] = build_graph(traffic_lights, t_min, t_max, tf, final_distance, ...
                                    v_min, v_max, b1, b2, start_position, time_offset)
    d = [traffic_lights.distance]; 
    ...
    for i = 1:nIntersections
        ...
        % Salta semafori già superati
        if light.distance <= start_position
            continue;
        end
        ...
        for k = 0:ceil(tf / light.cycle_time)
            ...
            % Gestione intervallo verde normale
            if light.green_start <= light.green_end
                ...
                if abs_green_start <= time_offset + tf
                ...
                if abs_green_start_2 <= time_offset + tf
        ...
        for j = 1:nNodes
            ...
            if lvlB == lvlA + 1
                if (Nodes(j).t > Nodes(i).t) && (Nodes(j).d > Nodes(i).d)
                    % Verifica semaforo verde per il nodo di arrivo
                    if lvlB > 0 && lvlB <= nIntersections && ~is_green(traffic_lights(lvlB), Nodes(j).t)
                        continue;
                    end
                    
                    % Calcolo velocità e costo
...
function platoon_vehicles = get_platoon_vehicles(run_i)
    ...
    % Trova i veicoli che appartengono a questo plotone
    ...
    if isfield(prev_run, 'splittedVehicles') && ~isempty(prev_run.splittedVehicles)
        ...
        % Se non ci sono veicoli divisi dal plotone precedente, è vuoto
        platoon_vehicles = [];
    end
end
...
function [split_times, split_vehicles, split_reasons] = find_splits_info()
    ...
    for i = 1:length(SIM_RUNS)-1
        if isfield(SIM_RUNS{i}, 'splittedVehicles') && ~isempty(SIM_RUNS{i}.splittedVehicles)
            ...
            % Determina tipo di split usando posizione
            if any([SIM_RUNS{1}.traffic_lights.distance] - SIM_RUNS{i+1}.startPos < 10 & ...
                  [SIM_RUNS{1}.traffic_lights.distance] - SIM_RUNS{i+1}.startPos > -10)
...
function plot_comparison()
    ...
    xlabel('Tempo [s]', 'FontSize', 12);
    ylabel('Posizione [m]', 'FontSize', 12);
    title('Confronto Traiettorie Reali vs Ottimali', 'FontSize', 14, 'FontWeight', 'bold');
    grid on;
end
...
function plot_leader_velocities()
    ...
    % Identificazione dei leader nelle simulazioni
    leaders = [];
    for run_i = 1:length(SIM_RUNS)
            ...
            if runData.leader == current_leader
                ...
                if isfield(runData, 'opt_t') && isfield(runData, 'opt_d')
                    ...
                    for i = 1:length(v_targets)
                        ...
                        v_segment = [v_targets(i), v_targets(i)];
                        if i == 1
    ...
    for run_i = 1:length(SIM_RUNS)
        ...
        if length(platoon_vehicles) >= 2
            ...
            for i = 1:length(sorted_vehicles)-1
                ...
                distance = x(:, v1) - x(:, v2);
                plot(t, distance, 'LineWidth', 2, 'DisplayName', ['Distanza ' num2str(v1) '-' num2str(v2)]);
            end
            
            title(['Distanze - Plotone ' num2str(run_i) ' (Leader ' num2str(leader) ')']);
            xlabel('Tempo [s]');
            ylabel('Distanza [m]');
            legend('Location', 'Best');
            grid on;
...
function follower_opt_d = calculate_follower_trajectory(follower_id, leader_id, opt_t, opt_d, v_targets)
    ...
    initial_offset = abs(follower_id - leader_id) * 4;
    ...
    for idx_opt = 2:length(opt_t)
        if follower_id > leader_id
            desired_gap = d_min + t_CTH*v_targets(min(idx_opt-1, length(v_targets)));
