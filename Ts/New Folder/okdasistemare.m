...
function final_plot()
    ...
    % Per ogni simulazione, plotta le traiettorie ottimali
    for run_i = 1:length(SIM_RUNS)
        runData = SIM_RUNS{run_i};
        
        % Controlla se i dati ottimali sono disponibili
        if isfield(runData, 'opt_t') && isfield(runData, 'opt_d')
            leader = runData.leader;
            
            % Plotta la traiettoria ottimale del leader
            ...
            legend_texts{end+1} = ['Leader ' num2str(leader) ' (Sim ' num2str(run_i) ')'];
            
            % Calcola le posizioni ottimali dei follower basandosi sul modello di platoon
            n_vehicles = size(runData.x, 2)/2;
            t_CTH = 1.5;  % Constant Time Headway
            d_min = 1;    % Distanza minima di sicurezza
            
            % Calcola le velocità target nei vari segmenti
            v_targets = [];
            ...
            % Aggiungi tutti i follower con colori e stili distintivi
            for v = 1:n_vehicles
                if v ~= leader
                    % Plotta solo i follower che non si sono staccati
                    if ~ismember(v, runData.splittedVehicles)
                        follower_opt_d = zeros(size(opt_d));
                        
                        % Il primo punto rimane offset iniziale
                        initial_offset = 4 * abs(v-leader);  % 4m distanza iniziale tra veicoli
                        follower_opt_d(1) = opt_d(1) - initial_offset * (v > leader) + initial_offset * (v < leader);
                        
                        % Per ogni segmento, calcola distanza progressiva considerando la velocità
                        for i = 2:length(opt_t)
                            if v > leader  % Follower dietro
                                % Distanza desiderata basata sulla velocità nel segmento precedente
                                desired_gap = d_min + t_CTH * v_targets(i-1);
                                follower_opt_d(i) = opt_d(i) - desired_gap * (v-leader);
                            else  % Follower davanti (casi speciali)
                                % Calcolo inverso per veicoli che precedono il leader
                                desired_gap = d_min + t_CTH * v_targets(i-1);
                                follower_opt_d(i) = opt_d(i) + desired_gap * (leader-v);
                            end
                        end
                        
                        % Usa colori e stili diversi per ogni follower
                        follower_color_idx = mod(v-1, length(colors))+1;
                        follower_line_idx = mod(v-1, length(line_styles))+1;
                        ...
                        legend_texts{end+1} = ['Follower ' num2str(v) ' (Sim ' num2str(run_i) ')'];
                    end
                end
            end
        end
    ...
    xlabel('Tempo [s]');
    ylabel('Posizione [m]');
    title('Traiettorie ottimali di leader e follower con stato semafori');
    grid on;
    ...
    % FIGURA 3: Profili di velocità di tutti i veicoli
    figure('Name','Profili di Velocità', 'Position', [200, 200, 1000, 600]);
    hold on;
    
    % Prepara colori e stili di linea diversi per ogni veicolo
    vehicle_colors = {'b', 'r', 'g', 'm', 'c', 'y', 'k'};
    line_styles = {'-', '--', ':', '-.'};
    
    % Prepara la matrice per le legende
    legend_handles = [];
    legend_texts = {};
    ...
    for run_i=1:length(SIM_RUNS)
        ...
        % Plotta velocità per ogni veicolo
        n_vehicles = size(runData.x, 2)/2;
        
        for v=1:n_vehicles
            if ismember(v, splitted)
                % Non plottare veicoli staccati in questa simulazione
                continue;
            end
            
            color_idx = mod(v-1, length(vehicle_colors))+1;
            line_idx = mod(run_i-1, length(line_styles))+1;
            
            % Ottieni la velocità del veicolo (dalla matrice x)
            velocity = runData.x(:, n_vehicles + v);
            
            % Plotta con colori/stili diversi
            h = plot(t, velocity, [vehicle_colors{color_idx}, line_styles{line_idx}], 'LineWidth', 2);
            
            % Aggiungi una label solo se questo veicolo è un leader o se è un veicolo nel plotone iniziale
            if v == runData.leader || (run_i == 1 && v ~= runData.leader)
                legend_handles(end+1) = h;
                if v == runData.leader
                    legend_texts{end+1} = ['Leader ' num2str(v) ' (Sim ' num2str(run_i) ')'];
                else
                    legend_texts{end+1} = ['Follower ' num2str(v) ' (Sim ' num2str(run_i) ')'];
                end
            end
        ...
        % Plotta anche il profilo target per il leader di questa simulazione
        if isfield(runData, 'opt_t') && isfield(runData, 'opt_d')
            leader = runData.leader;
            opt_t = runData.opt_t;
            opt_d = runData.opt_d;
            ...
            % Plot del profilo ideale come linea nera tratteggiata
            h_target = plot([opt_t(1:end-1); opt_t(2:end)], [opt_v; opt_v], 'k--', 'LineWidth', 1.5);
            
            % Aggiungi alla legenda, prendi solo il primo handle
            legend_handles(end+1) = h_target(1);
            legend_texts{end+1} = ['Target Leader ' num2str(leader) ' (Sim ' num2str(run_i) ')'];
            
            % Aggiungi punti ai nodi del percorso ottimale
            scatter(opt_t(1:end-1), opt_v, 50, 'k', 'filled');
        end
    end
    ...
    xlabel('Tempo [s]');
    ylabel('Velocità [m/s]');
    title('Profili di velocità di tutti i veicoli');
    grid on;
