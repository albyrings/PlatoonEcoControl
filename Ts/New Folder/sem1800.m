...
function run_optimizer_and_plot(leader_vehicle, time_offset, start_position)
    ...
    % Resetta le variabili persistenti di eventuali funzioni
    reset_persistent_variables();
    ...
    traffic_lights = [
         ...
         create_traffic_light(1800, 20, 30, T)
    ...
    % Nodi per i semafori (intervalli di verde)
    for i = 1:nIntersections
        ...
        for k = 0:ceil(tf / light.cycle_time)
            ...
            if light.green_start <= light.green_end
                ...
                % Caso in cui green_start > green_end e la "finestra verde" scavalca la fine del ciclo
    ...
    SIM_RUNS{end+1} = struct( ...
        ...
        'opt_t', opt_t, ...
        'opt_d', opt_d, ...
        'oldPlatoonTrajectory', [] ...  % Per ora vuoto nel primo run
    );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% La funzione seguente è stata modificata in modo che il nuovo plotone non
% si sovrapponga a quello vecchio. Anticipiamo le traiettorie precedenti
% e imponiamo un vincolo di distanza massima (non superare l’ultimo veicolo
% del plotone precedente, se ancora in “zona”).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function run_optimizer_from_traffic_light(leader_vehicle, start_time, start_position, ~)
    ...
    % Recupero la traiettoria “reale” del plotone precedente (se esiste)
    oldPlatoonTrajectory = get_old_platoon_trajectory();
    
    % Rimuovo i semafori già superati
    remaining_lights = [];
    ...
    % Costruzione nodi come prima...
    if isempty(remaining_lights)
        % se non ci sono semafori più avanti
            ...
            for j = 1:nNodes
                ...
                if lvlB == lvlA + 1
                    if (Nodes(j).t > Nodes(i).t) && (Nodes(j).d > Nodes(i).d)
                        ...
                        v_link = delta_d / delta_t;
                        if v_link >= v_min && v_link <= v_max
                            % -- Aggiunta del vincolo di non superamento del vecchio plotone
                            allowEdge = check_non_overlapping(Nodes(i).t, Nodes(j).t, ...
                                                              Nodes(i).d, Nodes(j).d, ...
                                                              oldPlatoonTrajectory);
                            if ~allowEdge
                                continue; % scarta l’arco
                            end
                            E_link = delta_t * (b1*v_link + b2*v_link^2);
                            Edges(edgeCount) = struct('from', Nodes(i).id, 'to', Nodes(j).id, 'w', E_link);
                            edgeCount = edgeCount + 1;
    ...
    SIM_RUNS{end+1} = struct( ...
        ...
        'opt_t', opt_t, ...
        'opt_d', opt_d, ...
        'oldPlatoonTrajectory', oldPlatoonTrajectory ...
    );
...
function oldPlatoon = get_old_platoon_trajectory()
    ...
    if isempty(SIM_RUNS)
        ...
        % Recupera l’ultimo run (quello appena completato)
        oldPlatoon = struct('t', SIM_RUNS{end}.t, 'x', SIM_RUNS{end}.x);
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function allow = check_non_overlapping(tA, tB, dA, dB, oldPlatoon)
    allow = true;
    if isempty(oldPlatoon)
        return;
    end
    
    times_old = oldPlatoon.t;
    x_old = oldPlatoon.x;
    n_vehicles = size(x_old,2)/2;
    
    % Nel caso più semplice imponiamo che in ogni tempo in [tA, tB], la
    % posizione della new platoon non superi l’ultimo veicolo “attivo” 
    % del plotone precedente (ad es. leader).
    % Per semplificare controlliamo solo i tempi tA e tB:
    t_check = [tA, tB];
    
    for tt = t_check
        if tt <= times_old(end)
            % Interpolo la posizione del veicolo leader del vecchio plotone
            old_leader_pos = interp1(times_old, x_old(:, 1), tt, 'linear', 'extrap');
            % Confronto se dA,dB superano old_leader_pos
            if (tt == tA && dA > old_leader_pos) || ...
               (tt == tB && dB > old_leader_pos)
                allow = false;
                return;
            end
        end
...
function plot_optimal_trajectories_and_lights()
    global SIM_RUNS
    if isempty(SIM_RUNS)
        disp('[plot_optimal_trajectories_and_lights] No data in SIM_RUNS.');
        return;
    end
    
    % Create figure
    figure('Name','Optimal Trajectories','Position',[100,100,900,600]);
    hold on; grid on;
    ...
    % Define standard colors for vehicles (consistent across plots)
    veh_colors = [
        0.0, 0.0, 1.0;   % blue (vehicle 1)
        1.0, 0.0, 0.0;   % red (vehicle 2)
        0.0, 0.8, 0.0;   % green (vehicle 3)
        0.8, 0.0, 0.8;   % magenta (vehicle 4)
        1.0, 0.5, 0.0    % orange (vehicle 5)
    ];
    
    legend_handles = [];
    legend_labels = {};
    
    % Plot each platoon's optimal trajectories
    for run_idx = 1:length(SIM_RUNS)
        runData = SIM_RUNS{run_idx};
        if ~isfield(runData, 'opt_t') || ~isfield(runData, 'opt_d')
            continue;
        end
        
        leader_id = runData.leader;
        opt_t = runData.opt_t;
        opt_d = runData.opt_d;
        
        % Plot leader with dashed line
        color_leader = veh_colors(mod(leader_id-1, size(veh_colors,1))+1, :);
        h_leader = plot(opt_t, opt_d, '--', 'Color', color_leader, 'LineWidth', 2);
        
        legend_handles(end+1) = h_leader;
        legend_labels{end+1} = ['Leader ' num2str(leader_id) ' (Optimal, Platoon ' num2str(run_idx) ')'];
        
        % Plot followers with dashed lines
        platoon_vehicles = get_platoon_vehicles(run_idx);
        for v = platoon_vehicles
            if v == leader_id, continue; end
            
            % Compute follower trajectory based on leader's optimal path
            color_v = veh_colors(mod(v-1, size(veh_colors,1))+1, :);
            offset = -4 * (v - leader_id);  % spacing of 4m
            follower_d = opt_d + offset;
            
            h_follower = plot(opt_t, follower_d, '--', 'Color', color_v, 'LineWidth', 2);
            legend_handles(end+1) = h_follower;
            legend_labels{end+1} = ['Vehicle ' num2str(v) ' (Optimal, Platoon ' num2str(run_idx) ')'];
        end
    end
    
    % Set axis limits
    xlim([0, 250]);  % Extended time range to show complete trajectories
    ylim([0, 2000]); % Based on final_distance
...
function plot_real_trajectories()
    ...
    % Create figure
    figure('Name','Real Trajectories and Traffic Lights','Position',[200,150,900,600]);
    hold on; grid on;
    ...
    % Define standard colors for vehicles (same as in optimal trajectories)
    veh_colors = [
        0.0, 0.0, 1.0;   % blue (vehicle 1)
        1.0, 0.0, 0.0;   % red (vehicle 2)
        0.0, 0.8, 0.0;   % green (vehicle 3)
    ...
    % Plot each platoon's real trajectories
    for run_idx = 1:length(SIM_RUNS)
        runData = SIM_RUNS{run_idx};
        if isempty(runData.x), continue; end
        
        t_sim = runData.t;
        x_sim = runData.x;
        leader_id = runData.leader;
        
        % Identify vehicles in this platoon
        platoon_vehicles = get_platoon_vehicles(run_idx);
        
        % Plot each vehicle's trajectory with solid line
        for v = platoon_vehicles
            color_v = veh_colors(mod(v-1, size(veh_colors,1))+1, :);
            pos_v = x_sim(:, v);
            
            h_vehicle = plot(t_sim, pos_v, '-', 'Color', color_v, 'LineWidth', 2);
            legend_handles(end+1) = h_vehicle;
            
            if v == leader_id
                legend_labels{end+1} = ['Leader ' num2str(v) ' (Real, Platoon ' num2str(run_idx) ')'];
            else
                legend_labels{end+1} = ['Vehicle ' num2str(v) ' (Real, Platoon ' num2str(run_idx) ')'];
            end
        end
    end
...
% Helper function (unchanged from original code)
%%%% Helper functions %%%%

function [max_time, max_distance] = get_global_time_and_distance()
    global SIM_RUNS
    max_time     = 0;
    max_distance = 0;
    for idxRun = 1:length(SIM_RUNS)
        runData = SIM_RUNS{idxRun};
        if ~isempty(runData.t)
            max_time = max(max_time, max(runData.t));
        end
        if isfield(runData,'x') && ~isempty(runData.x)
            pos_matrix = runData.x(:,1:(size(runData.x,2)/2));
            max_distance = max(max_distance, max(pos_matrix,[],'all'));
        end
    end
end

function col_map = define_vehicle_colors()
    col_map = [
        0.0, 0.0, 1.0;   % blue
        1.0, 0.0, 0.0;   % red
        0.0, 1.0, 0.0;   % green
...
function [all_times, all_distances, all_colors] = prepare_traffic_light_data(traffic_lights)
    ...
    for r = 1:length(SIM_RUNS)
        max_t = max(max_t, max(SIM_RUNS{r}.t));
    end
    times = 0:ceil(max_t);
    ...
    for i=1:length(traffic_lights)
        for j=1:length(times)
            time_j = times(j);
            all_times(end+1)     = time_j;
            all_distances(end+1) = traffic_lights(i).distance;
            if is_green(traffic_lights(i), time_j)
                all_colors(end+1,:) = [0,1,0]; % green
            else
                all_colors(end+1,:) = [1,0,0]; % red
            end
        end
    end
end
...
function platoon_vehicles = get_platoon_vehicles(run_i)
    ...
    if run_i == 1
        % All vehicles in the first run, except any splitted ones
        ...
        current_run = SIM_RUNS{run_i};
        leader = current_run.leader;
        ...
        if isfield(prev_run,'splittedVehicles') && ~isempty(prev_run.splittedVehicles)
            ...
            if isfield(current_run,'splittedVehicles') && ~isempty(current_run.splittedVehicles)
                platoon_vehicles = setdiff(platoon_vehicles, current_run.splittedVehicles);
            end
        else
            platoon_vehicles = 1:n_vehicles;
        end
        ...
        if isempty(platoon_vehicles)
            % If none remain, at least the new leader is in the run
            platoon_vehicles = leader;
        end
    end
end