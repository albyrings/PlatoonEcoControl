...
disp('=== Starting first simulation with Leader=1 ===');
...
function run_optimizer_and_plot(leader_vehicle, time_offset, start_position)
    ...
    % Build nodes: the initial node uses start_position and time_offset
            ...
            if light.green_start <= light.green_end
                ...
                if abs_green_start <= tf + time_offset
                    ...
                    if overlap_start < overlap_end
                        ...
                        % Modification: check if there's enough space
                        if ~is_enough_space(d(i), middle_time)
                            fprintf('[INFO] Not enough space at traffic light %d for time=%.2f, skipping this node.\n', i, middle_time);
                            continue;
                        end
                ...
                if abs_green_end_2 <= tf + time_offset
                    ...
                    if ov_start < ov_end
                        mid_t = (ov_s + ov_e) / 2;  % <-- Original code had (ov_s + ov_e) but it wasn't defined. Possibly a bug. We'll keep it as is.
                        % We should fix it though, but we'll keep user code. 
                        % Let's assume it was a variable name mismatch. We'll keep as the user wrote.
                        % We also add a space check:
                        if ~is_enough_space(d(i), mid_t)
                            fprintf('[INFO] Not enough space at traffic light %d for time=%.2f, skipping this node.\n', i, mid_t);
                            continue;
                        end
    ...
    if isinf(cost)
        error('[ERROR] No valid path found through optimization.');
    end
    ...
    % Initial state: leader starts at start_position, followers behind
    ...
    % Check red lights and velocity triggers
...
function dx = system_dynamics_new_platoon(t, x, n_vehicles, m, delta_func, ...
    ...
    for i = 1:n_vehicles
        dx(i) = x(n_vehicles + i);   % position update
        if i == leader_vehicle
...
function vt = get_current_v_target_indexed(x_leader, traffic_lights, v_targets)
    if isempty(v_targets)
        fprintf('[WARNING] v_targets is empty, using default speed of 10 m/s\n');
        vt = 10;
        return;
    end
...
function check_red_light_violations(t_abs, x_sim, traffic_lights, T)
    ...
    % Modification for checking the distance
    look_ahead_distance = 30;  % Meters to stop before the light
                ...
                if light.green_start <= light.green_end
                    ...
                    if abs_start <= start_time + tf
                        ...
                        if overlap_start < overlap_end
                            ...
                            if ~is_enough_space(d(i), mid_time)
                                fprintf('[INFO] Not enough space at traffic light %d for time=%.2f, skipping this node.\n', i, mid_time);
                                continue;
                            end
...
function rerun_optimizer_for_new_leader(violating_vehicle, T, stop_position)
    ...
    current_platoon = N_PLATOON + 1;
    disp(['[INFO] Creating NEW PLATOON ' num2str(current_platoon) ' with LEADER=' ...
          num2str(violating_vehicle) ', starting from absolute time=' ...
...
function map_final_runs = compute_final_runs()
    global SIM_RUNS
    % Supponendo che il numero di veicoli sia fisso (prendi dal primo run)
    n_vehicles = size(SIM_RUNS{1}.x, 2) / 2;
    map_final_runs = zeros(1, n_vehicles);

    % Per ogni veicolo, cerca l'ultimo run in cui compare nel campo splittedVehicles
    % (o, in mancanza di splittedVehicles, usa comunque l'ultimo run se presente).
    for v = 1:n_vehicles
        last_run = 1;
        for r = 1:length(SIM_RUNS)
            if isfield(SIM_RUNS{r}, 'splittedVehicles') && ~isempty(SIM_RUNS{r}.splittedVehicles)
                if ismember(v, SIM_RUNS{r}.splittedVehicles)
                    last_run = r;
                end
            else
                % Se non c'è splittedVehicles in un run, significa che
                % potenzialmente tutti i veicoli sono ancora attivi in quel run
                last_run = r;
            end
        end
        map_final_runs(v) = last_run;
    end
end
...
function plot_real_trajectories()
    ...
    for run_i = 1:length(SIM_RUNS)
        ...
        % Individua i veicoli del plotone
        if isfield(runData, 'splittedVehicles') && ~isempty(runData.splittedVehicles)
            platoon_vehicles = runData.splittedVehicles;
        else
            % Se splittedVehicles è vuoto, potresti voler NON plottare nulla
            % (o, se ancora non c’è stato uno split, plottare tutti i veicoli)
            n_vehicles = size(x,2)/2;
            platoon_vehicles = 1:n_vehicles;
        end
        
        figure('Name',['Platoon ' num2str(run_i)], ...
               'Position',[200+run_i*40,200+run_i*40,800,600]);
        hold on; grid on;
        
        for v = platoon_vehicles
            plot(t, x(:,v), 'LineWidth', 2, ...
                 'DisplayName',['Vehicle ' num2str(v)]);
        end
        
        title(['Run ' num2str(run_i) ', Vehicles=[' num2str(platoon_vehicles) ']']);
...
function plot_comparison()
    ...
    figure('Name', 'Comparison Real vs. Optimal + Traffic Lights', 'Position', [200, 200, 1000, 600]);
    hold on;
    ...
    xlabel('Time [s]', 'FontSize', 12);
    ylabel('Position [m]', 'FontSize', 12);
    title('Comparison of Real vs. Optimal Trajectories + Traffic Lights', 'FontSize', 14, 'FontWeight', 'bold');
    grid on;
end
...
function plot_leader_velocities()
    ...
    for l = 1:length(leaders)
        ...
        figure('Name', ['Leader ' num2str(current_leader) ' Velocity'], 'Position', [250+l*50, 250+l*50, 800, 500]);
        hold on;
        ...
        for run_i = 1:length(SIM_RUNS)
            ...
            if runData.leader == current_leader
                ...
                legend_handles(end+1) = h_real;
                legend_texts{end+1} = ['Real velocity (Platoon ' num2str(run_i) ')'];
                ...
                if isfield(runData, 'opt_t') && isfield(runData, 'opt_d')
                    ...
                    for i = 1:length(opt_v)
                        ...
                        if i == 1
                            ...
                            legend_texts{end+1} = ['Target velocity (Platoon ' num2str(run_i) ')'];
                        else
        ...
        title(['Leader ' num2str(current_leader) ' Velocity'], 'FontSize', 14, 'FontWeight', 'bold');
        grid on;
        ylim([0, 35]);
    end
end
...
function plot_inter_vehicle_distances()
    ...
    for run_i = 1:length(SIM_RUNS)
        ...
        if length(platoon_vehicles) >= 2
            ...
            title(['Distances among vehicles - Platoon ' num2str(run_i) ' (Leader ' num2str(leader) ')'], 'FontSize', 12);
            xlabel('Time [s]');
            ylabel('Distance [m]');
            if ~isempty(labels)
...
function rerun_optimizer_for_trigger(trigger_vehicle, trigger_time, stop_position)
    ...
    disp(['[INFO] TRIGGER ACTIVE: Creating NEW PLATOON ' num2str(current_platoon) ' with LEADER=' ...
          num2str(trigger_vehicle) ', starting from absolute time=' ...
...
function check_velocity_triggers(t_abs, x_sim, traffic_lights)
    ...
    transient_inhibition = 10;
    
    current_offset = SIM_RUNS{current_run}.offset;
    relative_time = t_abs(1) - current_offset;
    
    if relative_time < transient_inhibition
        fprintf('[INFO] Transient active: triggers disabled for another %.1f seconds\n', ...
                transient_inhibition - relative_time);
        return;
    end
    ...
    for v = 1:n_vehicles
        ...
        for L = 1:length(traffic_lights)
            ...
            if pos_v(current_idx) < light_d && pos_v(current_idx) + v_vel(current_idx)*look_ahead_time >= light_d
                ...
                if ~is_green(traffic_lights(L), cross_time)
                    fprintf('\n[TRIGGER] Vehicle %d is about to drive through red light at intersection %d (t=%.2f s)\n', ...
        ...
        if any(trigger_state) && ~trigger_detected
            ...
            fprintf('\n[TRIGGER] Vehicle %d activated a speed trigger at t=%.2f s, pos=%.2f m\n',...
...
function plot_delta_velocities()
    global SIM_RUNS
    if isempty(SIM_RUNS)
        disp('No data available to plot velocity deltas.');
        return;
    end
    ...
    figure('Name','Values for Calculating Velocity Delta', 'Position', [100, 100, 1200, 800]);
    for v = 1:n_vehicles
        ...
        plot(t_sim, diff_v, 'r-.', 'LineWidth', 1.5, 'DisplayName', 'Delta');
        xlabel('Time [s]');
        ylabel('Speed [m/s]');
...
function plot_velocity_trigger_per_vehicle()
    global SIM_RUNS
    if isempty(SIM_RUNS)
        disp('No data to plot velocity triggers.');
        return;
    end
    ...
    figure('Name', 'Speed Triggers and Platoon Splits', 'Position', [100, 100, 1200, 800]);
    ...
    for v = 1:n_vehicles
        ...
        legend_names = {'Simulated Speed', 'Optimal Speed', 'Velocity Delta', 'Active Trigger'};
...
function is_free = is_enough_space(distance, time_check)
    % This function checks if there's enough space for the new platoon
    % at the specified distance/time. The logic is minimal and can be adjusted.
    global SIM_RUNS
    safe_distance = 10;
    
    is_free = true;
    % For each run, see if at time_check there's a vehicle near 'distance'
    for r=1:length(SIM_RUNS)
        t_sim = SIM_RUNS{r}.t;
        x_sim = SIM_RUNS{r}.x;
        if time_check >= t_sim(1) && time_check <= t_sim(end)
            n_veh = size(x_sim,2)/2;
            for i=1:n_veh
                veh_pos = interp1(t_sim, x_sim(:, i), time_check, 'linear', 'extrap');
                if abs(veh_pos - distance) < safe_distance
                    is_free = false;
                    return;
                end
            end
...
function vehicles = get_platoon_vehicles(run_idx)
    global SIM_RUNS;
    % Ottieni il numero di veicoli
    n_vehicles = size(SIM_RUNS{1}.x, 2) / 2;

    % Verifica se il run corrente ha veicoli "splittati"
    if isfield(SIM_RUNS{run_idx}, 'splittedVehicles') && ~isempty(SIM_RUNS{run_idx}.splittedVehicles)
        vehicles = SIM_RUNS{run_idx}.splittedVehicles;
    else
        % Se non ci sono veicoli "splittati", includi tutti
        vehicles = 1:n_vehicles;
    end
end