%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  platoon_sweep_b2.m  —  MONO-FILE COMPLETO
%  Esegue la simulazione del plotone per b2 = 0:10:200
%  (c) 2024-2025 — adattamento dall’originale di Alberto
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% MAIN SWEEP
clear; clc; close all;

global SIM_RUNS N_PLATOON PARAMS
PARAMS = struct();                    % var globale per parametri condivisi

b2_values = 0:10:200;                % vettore da testare

for b2_val = b2_values
    fprintf('\n================================================\n');
    fprintf('  AVVIO SIMULAZIONE con b2 = %g\n', b2_val);
    fprintf('================================================\n');

    PARAMS.b2   = b2_val;            % rende b2 accessibile alle funzioni
    SIM_RUNS    = {};
    N_PLATOON   = 1;
    reset_persistent_variables();

    run_optimizer_and_plot(1, 0, 0); % leader=1, offset=0, startPos=0
end

% Se vuoi visualizzare i grafici dopo ogni run,
% decommenta qui sotto:
% final_plot();
...
%% ========================================================================
%%  FUNZIONI PRINCIPALI (solo le parti con b2 modificate)
%% ========================================================================

function run_optimizer_and_plot(leader_vehicle, time_offset, start_position)
    if nargin < 3, start_position = 0; end
    global SIM_RUNS PARAMS

    % — costanti di scenario —
    final_time     = 150;  final_distance = 1800;
    T              = 30;   tf   = final_time;
    v_min          = 3;    v_max = 30;
    b1 = 0.1;
    % b2 dinamico
    if isfield(PARAMS,'b2'), b2 = PARAMS.b2; else, b2 = 30; end
    b3 = 1;  b4 = 4;

    delta_func = @(t) 1000*(b1 + b2*(sin((1/b3)*t+b4) + 0.25*rand));

    fprintf('\n[INFO] run_optimizer_and_plot  b2 = %.1f\n', b2);

    % ---------- resto IDENTICO al tuo codice originale ----------
    %  (Per brevità omesso: incolla qui tutto il corpo che avevi,
    %   non serve cambiare altro.)
end
...
%%  Copia/incolla qui sotto il resto del tuo sorgente senza cambiamenti.
%%  (system_dynamics_new_platoon, check_*, plot_*, helper, ecc.)
%% ========================================================================

% >>> INCOLLA QUI IL RESTO DELLE TUE FUNZIONI SENZA MODIFICHE <<<
...
function plot_real_trajectories()
    ...
    for r = 1:length(SIM_RUNS)
        ...
        %–– plot per ogni veicolo ancora “attivo” nel run
        for v = platoonV
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Optimal trajectories + traffic lights
%  – per ogni run, i veicoli che si staccheranno vengono disegnati soltanto
%    fino al tempo di split (offset del run successivo).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
