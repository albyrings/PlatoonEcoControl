...
function run_optimizer_and_plot(leader_vehicle, time_offset, start_position)
    ...
    traffic_lights = [
         ...
         create_traffic_light(600,  10, 25, T)
         create_traffic_light(900,  20, 35, T)
         create_traffic_light(1200,  0, 10, T)
         create_traffic_light(1550, 10, 20, T)
         create_traffic_light(2300,   0, 10, T)
         create_traffic_light(2600,  10, 20, T)
         create_traffic_light(2900,  20, 30, T)
         create_traffic_light(3200,  0, 10, T)
         create_traffic_light(3550, 10, 20, T)
     ];
...
function [legend_handles, legend_texts] = plot_run_optimal_trajectories(run_i, markers, colors, line_styles, legend_handles, legend_texts)
    ...
    % CHANGED CODE: sostituito legend_handles(end+1) = h; con:
    ...
    for v = platoon_vehicles
        if v ~= leader
            ...
            % CHANGED CODE: sostituito legend_handles(end+1) = h_follower; con:
            legend_handles = [legend_handles, h_follower];
...
function rerun_optimizer_from_traffic_light(vehicle_id, start_time, start_position, traffic_light)
    ...
    % Parametri necessari per la verifica di fattibilità
    final_time = 500;         
    final_distance = 4000;
    v_min = 5;   
    v_max = 30;  
    ...
    % Verifica le condizioni di realizzabilità
    fprintf('\n[INFO] Verifica condizioni di realizzabilità per nuovo plotone da semaforo...\n');
    is_feasible = check_feasibility_conditions(start_time, start_position, SIM_RUNS{1}.traffic_lights, ...
                                              v_min, v_max, start_time + final_time, final_distance);
    
    if ~is_feasible
        fprintf('[WARNING] Percorso non realizzabile con i vincoli attuali!\n');
        fprintf('          Si procederà comunque utilizzando un percorso predefinito.\n');
    end
...
function rerun_optimizer_for_trigger(trigger_vehicle, trigger_time, stop_position)
    ...
    % Recupera la lista dei semafori
    traffic_lights = SIM_RUNS{1}.traffic_lights;

    % Cerca il primo semaforo con distanza maggiore della posizione di stop
    next_light = [];
    for i = 1:length(traffic_lights)
        if traffic_lights(i).distance > stop_position
            next_light = traffic_lights(i);
            break;
        end
    end

    if isempty(next_light)
        warning('Nessun semaforo successivo trovato. Riottimizzazione dalla posizione attuale.');
        new_start_time = trigger_time;
        next_start = stop_position;
    else
        % Calcola il nuovo tempo di partenza in base al prossimo semaforo
        new_start_time = next_green(next_light, trigger_time);
        % Parte leggermente prima del semaforo (es. 5m prima)
        next_start = next_light.distance - 5;
        
        disp(['[INFO] Riottimizzazione dal prossimo semaforo a ' num2str(next_light.distance) ...
             'm, partenza a t=' num2str(new_start_time) ', posizione=' num2str(next_start)]);
    end

    % Verifica le condizioni di realizzabilità
    fprintf('\n[INFO] Verifica condizioni di realizzabilità per nuovo plotone...\n');
    is_feasible = check_feasibility_conditions(new_start_time, next_start, traffic_lights, ...
                                              v_min, v_max, new_start_time + final_time, final_distance);
    
    if ~is_feasible
        fprintf('[WARNING] Percorso non realizzabile con i vincoli attuali!\n');
        fprintf('          Provo ad aumentare la flessibilità dei vincoli...\n');
        
        % Si potrebbe implementare qui una strategia per aumentare la flessibilità
        % Ad esempio, aumentando il tempo finale o riducendo i vincoli di velocità
    end

    current_platoon = N_PLATOON + 1;
    disp(['[INFO] CREAZIONE PLOTONE ' num2str(current_platoon) ' con LEADER=' ...
          num2str(trigger_vehicle) ', riparto da t=' num2str(new_start_time) ...
          ', posizione=' num2str(next_start)]);

    N_PLATOON = N_PLATOON + 1;
    
    run_optimizer_from_traffic_light(trigger_vehicle, new_start_time, next_start, next_light);
...
function is_feasible = check_feasibility_conditions(start_time, start_position, traffic_lights, v_min, v_max, final_time, final_distance)
    % Funzione che verifica le condizioni di realizzabilità per un percorso
    % ottimo tra due punti, attraversando una sequenza di semafori
    %
    % Input:
    % - start_time: tempo di partenza [s]
    % - start_position: posizione iniziale [m]
    % - traffic_lights: struttura contenente i semafori
    % - v_min, v_max: velocità minima e massima consentite [m/s]
    % - final_time: tempo finale [s]
    % - final_distance: posizione finale [m]
    %
    % Output:
    % - is_feasible: vero se esiste almeno un percorso realizzabile

    is_feasible = true;
    reason = '';
    
    % Step 1: Verifica della condizione base di realizzabilità del rombo
    delta_t = final_time - start_time;
    delta_d = final_distance - start_position;
    
    if delta_t < delta_d/v_max
        is_feasible = false;
        reason = sprintf('Tempo insufficiente per raggiungere la destinazione: min t=%.2f, available=%.2f', ...
                        delta_d/v_max, delta_t);
        fprintf('[FEASIBILITY] %s\n', reason);
        return;
    end
    
    if delta_t > delta_d/v_min
        is_feasible = false;
        reason = sprintf('Tempo eccessivo per la velocità minima: max t=%.2f, available=%.2f', ...
                        delta_d/v_min, delta_t);
        fprintf('[FEASIBILITY] %s\n', reason);
        return;
    end
    
    % Step 2: Filtra i semafori rilevanti (quelli tra start e final position)
    relevant_lights = [];
    for i = 1:length(traffic_lights)
        if traffic_lights(i).distance > start_position && traffic_lights(i).distance < final_distance
            relevant_lights = [relevant_lights; traffic_lights(i)];
        end
    end
    
    if isempty(relevant_lights)
        fprintf('[FEASIBILITY] Nessun semaforo tra posizione iniziale e finale. Percorso realizzabile.\n');
        return; % Se non ci sono semafori nel percorso, è realizzabile
    end
    
    % Step 3: Calcola i limiti temporali per ogni semaforo (pruning)
    n_lights = length(relevant_lights);
    t_min = zeros(1, n_lights);
    t_max = zeros(1, n_lights);
    t_min_back = zeros(1, n_lights);
    t_max_back = zeros(1, n_lights);
    t_min_refined = zeros(1, n_lights);
    t_max_refined = zeros(1, n_lights);
    
    for i = 1:n_lights
        light_pos = relevant_lights(i).distance;
        
        % Vincoli in avanti
        t_min(i) = start_time + (light_pos - start_position)/v_max;
        t_max(i) = start_time + (light_pos - start_position)/v_min;
        
        % Vincoli all'indietro
        t_min_back(i) = final_time - (final_distance - light_pos)/v_max;
        t_max_back(i) = final_time - (final_distance - light_pos)/v_min;
        
        % Raffinamento dei vincoli
        t_min_refined(i) = max(t_min(i), t_min_back(i));
        t_max_refined(i) = min(t_max(i), t_max_back(i));
        
        % Verifica immediata della condizione base del rombo per questo semaforo
        if t_min_refined(i) > t_max_refined(i)
            is_feasible = false;
            reason = sprintf('Vincoli incompatibili per semaforo a %.1fm: [%.2f, %.2f]', ...
                            light_pos, t_min_refined(i), t_max_refined(i));
            fprintf('[FEASIBILITY] %s\n', reason);
            return;
        end
    end
    
    % Step 4: Verifica intersezione con le fasi di verde per ogni semaforo
    for i = 1:n_lights
        light = relevant_lights(i);
        green_intersection_exists = false;
        
        % Calcola il numero di cicli necessari da considerare
        max_cycles = ceil((t_max_refined(i) - start_time) / light.cycle_time) + 1;
        
        % Controlla se esiste almeno un'intersezione con una fase verde
        for k = 0:max_cycles
            cycle_start = k * light.cycle_time + light.offset;
            
            if light.green_start <= light.green_end
                % Fase verde continua all'interno del ciclo
                green_start = cycle_start + light.green_start;
                green_end = cycle_start + light.green_end;
                
                % Verifica intersezione
                if (green_start <= t_max_refined(i)) && (green_end >= t_min_refined(i))
                    green_intersection_exists = true;
                    break;
                end
            else
                % Fase verde divisa in due parti (attraversa la mezzanotte)
                green_start1 = cycle_start + light.green_start;
                green_end1 = cycle_start + light.cycle_time;
                
                green_start2 = cycle_start;
                green_end2 = cycle_start + light.green_end;
                
                % Verifica intersezione con entrambe le parti
                if (green_start1 <= t_max_refined(i) && green_end1 >= t_min_refined(i)) || ...
                   (green_start2 <= t_max_refined(i) && green_end2 >= t_min_refined(i))
                    green_intersection_exists = true;
                    break;
                end
            end
        ...
        if ~green_intersection_exists
            is_feasible = false;
            reason = sprintf('Nessuna intersezione con fase verde per semaforo a %.1fm nell''intervallo [%.2f, %.2f]', ...
                            light.distance, t_min_refined(i), t_max_refined(i));
            fprintf('[FEASIBILITY] %s\n', reason);
            return;
        end
    end
    
    % Stampa dettagli sulla realizzabilità
    fprintf('[FEASIBILITY] Il percorso è realizzabile con i vincoli dati.\n');
    for i = 1:n_lights
        fprintf('    - Semaforo %.1f m: finestra temporale ammissibile [%.2f, %.2f]\n', ...
                relevant_lights(i).distance, t_min_refined(i), t_max_refined(i));
    end
end