...
function dx = system_dynamics_new_platoon(t, x, n_vehicles, m, delta_func, traffic_lights, v_targets, t_CTH, ...
                                           ...
                                           leader_vehicle, time_offset)
    % Sistema dinamico (ODE) per un plotone di veicoli
...
function vt = get_current_v_target_indexed(x_leader, traffic_lights, v_targets)
    % Seleziona il target di velocità in base alla posizione del leader
...
function check_red_light_violations(t_abs, x_sim, traffic_lights, T)
    % Verifica se un veicolo attraversa un semaforo rosso e riattiva l'ottimizzazione
...
function rerun_optimizer_for_new_leader(violating_vehicle, T)
    % Cancella alcune funzioni ODE e riparte la simulazione con un nuovo leader
...
function final_plot()
    ...
    for run_i = 1:length(SIM_RUNS)
        ...
        if isfield(runData, 'opt_t') && isfield(runData, 'opt_d')
            ...
            % Calcola velocità target tra i nodi ottimali (non utilizzato nel plot, ma mostrato per completezza)
            v_targets = [];
            ...
            % Filtra i veicoli del plotone in base a platoon_vehicles
...
function [path, cost] = dijkstra(Nodes, Edges, source, target)
    % Implementazione dell'algoritmo di Dijkstra per il percorso ottimo
    nNodes = length(Nodes);
    cost = inf(1, nNodes);
    prev = nan(1, nNodes);
    cost(source) = 0;
...
function reset_persistent_variables()
    % Pulisce le variabili persistenti usate nel calcolo della dinamica e altri metodi
    clear system_dynamics_new_platoon
    clear check_red_light_violations
    clear get_current_v_target_indexed
    clear next_green
...
function plot_delta_velocities()
    ...
    for v = 1:n_vehicles
        ...
        % Verifica che esista il profilo ottimale
        if ~isfield(runData, 'opt_t') || ~isfield(runData, 'opt_d')
            continue;
        end
        opt_t = runData.opt_t;
...
function trigger_events = velocity_trigger(t, diff_v, opt_v)
    ...
    % opt_v  : profilo della velocità ottimale (derivata della posizione ottimale)
...
function plot_velocity_trigger_per_vehicle()
    ...
    for v = 1:n_vehicles
        ...
        % Il trigger è scalarizzato per renderlo visibile sul grafico
