%%%% matlab
% filepath: /path/to/provaIntegrazione1.m
...
% Parametri per forze esterne (disattivato in questa demo)
b1_ext = 450; b2_ext = 450; b3_ext = 1; b4_ext = 3;
delta = @(t) 0 * (b1_ext + b2_ext*(sin((1/b3_ext)*t+b4_ext) + 0.25*rand));
...
for i = 1:n_vehicles
    plot(t_dyn, x(:, i), 'Color', colors(i,:), 'LineWidth',2, ...
        'DisplayName', ['Veicolo ' num2str(i)]);
end
xlabel('Tempo [s]');
ylabel('Posizione [m]');
...
function [t_sim, d_sim, v_sim] = simulate_vehicle_segmented(opt_nodes, dt, traffic_lights, init_speed, accel)
    ...
    for i = 2:length(opt_nodes)
        ...
        if currentSpeed > v_req
            ...
            if abs(t_vec_dec(end) - (currentTime + t_dec)) > 1e-6
                t_vec_dec(end+1) = currentTime + t_dec;
            end
            v_vec_dec = currentSpeed - accel * (t_vec_dec - currentTime);
            d_vec_dec = currentDist + currentSpeed * (t_vec_dec - currentTime) ...
                        - 0.5 * accel * (t_vec_dec - currentTime).^2;

            d_dec = d_vec_dec(end) - currentDist;
            d_rem = D_seg - d_dec; 
            t_const = d_rem / v_req; 
            ...
            if abs(t_vec_const(end) - (t_start_const + t_const)) > 1e-6
                t_vec_const(end+1) = t_start_const + t_const;
            end
            d_start_const = d_vec_dec(end);
            d_vec_const = d_start_const + v_req * (t_vec_const - t_start_const);
            v_vec_const = v_req * ones(size(t_vec_const));

            t_start_wait = t_vec_const(end);
            t_vec_wait = t_start_wait : dt : (t_start_wait + t_wait);
            if abs(t_vec_wait(end) - (t_start_wait + t_wait)) > 1e-6
                t_vec_wait(end+1) = t_start_wait + t_wait;
            end
            ...
            if abs(t_vec(end) - targetTime) > 1e-6
                t_vec(end+1) = targetTime;
            end
        ...
        % --- Attesa al semaforo se in rosso ---
        if opt_nodes(i).int>0 && opt_nodes(i).int<=length(traffic_lights)
            ...
            if ~is_green(light, currentTime)
                ...
                if abs(t_vec_wait2(end) - (currentTime + t_wait2)) > 1e-6
                    t_vec_wait2(end+1) = currentTime + t_wait2;
                end
                d_vec_wait2 = currentDist * ones(size(t_vec_wait2));
                v_vec_wait2 = zeros(size(t_vec_wait2));
                
                if numel(t_vec_wait2) > 1
                    t_seg_vect = [t_seg_vect, t_vec_wait2(2:end)];
                    d_seg_vect = [d_seg_vect, d_vec_wait2(2:end)];
                    v_seg_vect = [v_seg_vect, v_vec_wait2(2:end)];
                else
                    t_seg_vect = [t_seg_vect, t_vec_wait2];
                    d_seg_vect = [d_seg_vect, d_vec_wait2];
                    v_seg_vect = [v_seg_vect, v_vec_wait2];
                end
                currentTime = t_vec_wait2(end);
                currentDist = d_vec_wait2(end);
                currentSpeed = 0; 
            end
        end

        % Aggiorna i vettori globali
...
% - Follower: controllo PID sulla distanza dal veicolo anteriore
    ...
    for i = 1:n_vehicles
        ...
        dx(i) = x(n_vehicles+i);
        if i == 1
            % LEADER: controllo PID sulla velocità
