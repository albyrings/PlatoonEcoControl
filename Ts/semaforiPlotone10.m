...
global PLATOON_START_TIMES; % Array globale per tenere traccia dei tempi di inizio dei plotoni
SIM_RUNS = {};      % Ciascun elemento: struct con campi (t, x, offset, leader)
PLATOON_START_TIMES = [0]; % Inizializza con il tempo di inizio del primo plotone
            ...
            if ~isempty(cross_idx)
                ...
                if ~is_green(traffic_lights(L), cross_time)
                    ...
                    if ~new_leader_detected
                        ...
                        SIM_RUNS{last_idx}.splittedVehicles = 1:v; % Salva tutti i veicoli del plotone precedente

                        rerun_optimizer_for_new_leader(v, cross_time); % Passa il tempo di violazione
                    end
                    return;
                end
            end
...
function rerun_optimizer_for_new_leader(violating_vehicle, violation_time)
    % Azzera le persistent
    global SIM_RUNS;
    global PLATOON_START_TIMES;

    % Il nuovo plotone inizia al tempo di violazione
    PLATOON_START_TIMES = [PLATOON_START_TIMES, violation_time];
    start_offset = violation_time;

    disp(['[INFO] Ricalcolo con NUOVO LEADER=', num2str(violating_vehicle), ...
        ', riparto da tempo assoluto=', num2str(start_offset)]);

    run_optimizer_and_plot(violating_vehicle, start_offset); % Correzione qui!
end